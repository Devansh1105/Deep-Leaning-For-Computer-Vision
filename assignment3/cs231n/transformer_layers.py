...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        pe = torch.ones(1, max_len, embed_dim)
        ############################################################################
        ...
        # less than 5 lines of code.        L,E/2                (L,1)             (E/2,1)          #
        ############################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        pe[0,:,::2] = torch.sin(torch.arange(max_len, dtype=torch.float).unsqueeze(1) @ torch.pow(10000, - (torch.arange(0, embed_dim, 2))/embed_dim).unsqueeze(0))
        pe[0,:,1::2] = torch.cos(torch.arange(max_len, dtype=torch.float).unsqueeze(1) @ torch.pow(10000,- (torch.arange(1, embed_dim, 2) - 1)/embed_dim).unsqueeze(0))
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        scores = torch.matmul(q,k)/ math.sqrt(self.emd_dim/H)
        if attn_mask is not None:
          scores = scores.masked_fill(attn_mask.unsqueeze(0).unsqueeze(0) == 0, float('-inf'))
        Y = torch.matmul(self.attn_drop(F.softmax(scores ,dim = -1)), v)
